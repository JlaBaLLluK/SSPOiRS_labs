...
class Client:
    ...
    def download_handler(self, request):
        self.socket.send(request.encode())
        is_old_user = bool(int(read_socket(self.socket)))
        try:
            with open(request.split()[1], 'rb') as file:
                data_start_position = len(file.read()) if is_old_user else 0
        except FileNotFoundError:
            data_start_position = 0

        self.socket.send((str(data_start_position) + '\n').encode())
    ...
    def upload_handler(self, request):
        self.socket.send(request.encode())
        fs = FileService(request.split()[1], self.socket)
        data_start_position = int(read_socket(self.socket))
